...
public interface RestFiles {
	 ...
	 *               this will not be used).
	 * 
	 * @return 204 if success; 404 if the fileId does not exist. 403 if the token is
	 *         invalid. 400 otherwise.
	 */
	@DELETE
	@Path("/{fileId}")
	void deleteFile(@PathParam("fileId") String fileId, @QueryParam("token") @DefaultValue("") String token);
	 ...
	 *         if the fileId does not exist. 403 if the token is invalid. 400
	 *         otherwise.
	 */
	@GET
	@Path("/{fileId}")
	@Produces(MediaType.APPLICATION_OCTET_STREAM)
	byte[] getFile(@PathParam("fileId") String fileId, @QueryParam("token") @DefaultValue("") String token);
}
